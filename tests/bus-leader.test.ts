...
test("Same-cwd followers keep sticky directory labels while rename updates only named mode", async () => {
  ...
  try {
    ...
    const response = await sendTelegramBusLocalEnvelope({ socketPath, envelope: {
      ...
      registrationGeneration: "follower:1", threadName: "Navigator", sentAtMs: 2,
    } });
    assert.ok(response?.kind === "bus.ack");
    ...
    assert.equal(renamed?.threadName, "Navigator");
    assert.equal(renamed?.displayTitle, "extensions_b");
    assert.equal(registry.get("follower")?.threadName, "Navigator");
    await runtime.setThreadDisplayMode!("names");
    assert.deepEqual(titles, ["extensions_a", "extensions_b", "Anchor", "Navigator"]);
    assert.equal(renamed?.target.threadId, 42);
    assert.equal(renamed?.slot, "B");
    await runtime.setThreadDisplayMode!("directories");
    assert.deepEqual(titles.slice(-2), ["extensions_a", "extensions_b"]);
