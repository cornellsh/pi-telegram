...
import { createTelegramThreadDisplayReconciler } from "./thread-display.ts";
  ...
  const runtime = createTelegramBusLeaderRuntime({
    ...
    async renameFollowerThread(follower, threadName) {
      ...
      const rename = Threads.createTelegramTopicTargetRenamer({
        store: deps.topicTargetStore,
        callApi: deps.callApi,
        shouldRenameDisplayedTitle: () => (deps.getThreadDisplayMode?.() ?? "names") === "names",
        assertAuthority() {
          if (
            deps.getCurrentLeaderEpoch &&
            deps.getCurrentLeaderEpoch() !== leaderEpoch
      ...
      await deps.topicTargetStore.persist();
      return { threadName: renamed.threadName ?? threadName };
      });
    },
    ...
    > => {
      ...
      if (workspaceIdentity) {
        const workspaceCommit =
          deps.topicTargetStore.upsertWorkspaceBinding(
            {
              ...workspaceIdentity,
              target: { ...result.target },
