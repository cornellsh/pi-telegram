...
export function normalizeTelegramWorkspacePath(
  ...
  const normalized = resolve(trimmed).replaceAll("\\", "/");
...
export interface TelegramWorkspaceThreadBinding {
  ...
  bindingKey: string;
  target: TelegramTarget & { threadId: number };
  /** Stable named-mode identity, independent of the current display title. */
  threadName?: string;
  slot?: string;
    ...
    renameByTarget(target, threadName, options) {
      ...
      for (const record of records.values()) {
        ...
        for (const binding of workspaceBindings.values()) {
          ...
          const previousTitle = binding.displayTitle ?? binding.threadName;
          binding.threadName = normalizedThreadName;
