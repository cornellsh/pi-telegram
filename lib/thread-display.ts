...
import type { TelegramTopicTargetStore, TelegramWorkspaceThreadBinding } from "./threads.ts";
import type { TelegramApiCallOptions } from "./telegram-api.ts";

type DisplayBinding = Pick<TelegramWorkspaceThreadBinding,
  "bindingKey" | "cwd" | "slot" | "threadName" | "showSlotSuffix">;
...
export function resolveTelegramWorkspaceDisplayNames(
  bindings: readonly DisplayBinding[],
  ...
  if (mode === "directories") {
    ...
    for (const binding of bindings) {
      ...
      if (!label || (counts.get(label.toLowerCase()) ?? 0) < 2) continue;
