...
- `Workspace Admission`: Preserves saved names over follower hints during dormant recovery and target replacement. Explicit same-directory connection skips live peer bindings and allocates the next identity without copying their targets; startup restore remains allocation-free. Regressions cover competing names and concurrent claims. Unchanged snapshots now skip directory preparation and temporary-name generation while retaining disk comparison and exact-owner commits.
- `Workspace Continuity`: Authority fails closed across routing, API, cleanup, journals, and process ownership. Intents/fences block mutation and slot reuse; the ledger prunes only proven-dead leases. Execution emits one deletion permit, never reissues unknown deletion, and commits only after confirmed absence. Cleanup, lifecycle, routing, and deferred reconciliation use admitted gating; concurrent operation-ID reuse is rejected. Retirement is pressure-only, disconnected, and never triggered by time.
- `Workspace Recovery`: Requires visibility proof for dormant targets; cold multi-binding recovery without exact handoff follows current opening order. Confirmed-dead cleanup retains the Workspace name and ordering slot while removing routing authority. A stale-target reopen replaces only the deleted target and carries its identity forward. `/telegram-connect [profile] as=Name` names a fresh Workspace; `/telegram-name Name` performs collision-safe leader-owned rename.
- `Follower Auto-Connect`: Reconnects a reopened follower automatically when its selected profile remembers an exact-working-directory Workspace Thread and another live leader owns polling. Capability-negotiated restore-only admission may reclaim, visibility-probe, or replace that binding, but cannot allocate a Thread for a new Workspace; explicit `/telegram-connect` remains the provisioning boundary.
- `Mobile Explanations`: Bundles the portable Show Me Skill with pi-telegram and adapts current-work explanations to phone-width Markdown or self-contained browser-ready HTML. It separates local, validated, released, and live state; maps visual diffs to actual mechanisms; and refuses to infer clickability or client behavior from source shape alone.
- `Thread Display`: Adds profile-scoped Letters, Names (default), and Directories with leader-owned fenced persistence and partial-failure reporting. Non-default modes reject incompatible peers; Names remains recoverable. Manual rename preserves non-named titles, setup preserves concurrent preference changes, and generation-bound acknowledged titles drive TUI, bot chooser/notice, prompt attribution, and live agent-target labels without replacing numeric routing authority.
- `Command Legibility`: Keeps Pi TUI commands in Telegram HTML as monospace code and bot commands plain and hyphen-free. Semantic Rich replies, previews, guest answers, and attachment captions permit native command/URL detection; technical thinking/tool evidence retains literal entity suppression.
- `Follower Availability`: Keeps transiently delayed follower heartbeats routable for 15 seconds instead of five, identifies an unregistered follower as reconnecting, and tells Telegram operators to retry before reconnecting while preserving the Thread. Local IPC now consumes acknowledgements already buffered during a blocked follower event loop before declaring timeout, avoiding false `ETIMEDOUT` / peer-reset recovery churn while genuinely silent peers remain bounded.
