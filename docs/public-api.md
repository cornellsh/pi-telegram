...
Threaded Mode Settings exposes **Thread display** in the order Letters, Names (default), Directories. `profiles.<name>.threadDisplayMode` is profile-scoped; absent or invalid values resolve to `names`. The leader serializes preference persistence and title reconciliation, while a follower sends an authenticated `follower.setThreadDisplayMode` request gated by `thread-display-mode-v1` and its exact registration generation. Changing to Letters or Directories requires compatible connected followers; returning to Names remains available with legacy peers. Non-default modes reject incompatible registration before provisioning and recheck compatibility before live publication. Config writes check the originating authority inside the config transaction; mode changes preserve target IDs, slots, saved names, and queue ownership. The caller confirms only after application succeeds. A partial failure may leave the preference saved and some titles updated; Settings reports that state and permits retry. Acknowledged follower titles arrive through heartbeat rather than a new read loop.
